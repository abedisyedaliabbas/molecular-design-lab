...
// Statistics: 80+ cities visited, 19 countries/regions visited
...
Evenings in London had their own charm. The city's lights reflected off the Thames as I joined the throngs of people making their way to West End shows or to one of the many historic pubs to enjoy a pint and perhaps, engage in lively debates with locals and fellow travelers. These moments of connection reminded me that while my research could be isolating, the world outside was always waiting, vibrant and alive.

Londoners, I found, were particularly spirited on game days. Whenever there was a match at Wembley Stadium, the city transformed. The streets brimmed with fans donned in their team colors, their chants and cheers creating a palpable buzz that could be felt across the city. Running on these days brought a different kind of joy, as I maneuvered through waves of fans streaming towards the stadium, their camaraderie and passion infusing the air with electricity.
...
// Render travel blog posts with full narratives
function renderTravelBlogPosts() {
  const container = document.getElementById("travelBlogPosts");
  if (!container) return;

  // Sort by date (most recent first) - simple string comparison for dates
  const sortedPosts = [...travelBlogPosts].sort((a, b) => {
    // Extract year for comparison
    const yearA = parseInt(a.date.match(/\d{4}/)?.[0] || "0");
    const yearB = parseInt(b.date.match(/\d{4}/)?.[0] || "0");
    if (yearB !== yearA) return yearB - yearA;
    // If same year, reverse order to show newer dates first
    return travelBlogPosts.indexOf(b) - travelBlogPosts.indexOf(a);
  });
            ...
            <div class="d-flex align-items-start gap-3 mb-3">
              <div>
                ...
                <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                  <span class="badge bg-primary-subtle text-primary">${post.country}</span>
                  <span class="text-muted small"><i class="fa-solid fa-calendar me-1"></i>${post.date}</span>
                </div>
              </div>
            </div>
